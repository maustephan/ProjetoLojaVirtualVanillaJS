...
function buscarProdutos(e){
    ...
    // let consulta = produtos.filter((consultaProduto) => consultaProduto.nome === nomeProduto);
        ...
        if ( consulta ){
            ...
            fetch(url, {
                ...
                body: JSON.stringify({
                    ...
                    imagem: imagemProduto.value.toLowerCase(),
...
const btnCep = document.getElementById('buscaCep');
...
btnCep.addEventListener('click', () => {
    ...
    .then( (consulta) => {
        ...
        if (consulta.erro){
            div.innerHTML = `<br><h3>CEP NÃO EXISTE!!!</h3>`
        } else {
            div.innerHTML = `<div><br />
                CEP: ${consulta.cep}<br />
                Rua: ${consulta.logradouro}<br />
                Bairro: ${consulta.bairro}<br />
                Estado: ${consulta.localidade}<br />
                UF: ${consulta.uf}<br /><br />
